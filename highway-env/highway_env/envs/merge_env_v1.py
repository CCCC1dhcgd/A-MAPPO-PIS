...
#             # hard mode: 3-6 CAVs + 3-6 HDVs
#             if num_CAV == 0:
#                 num_CAV = np.random.choice(np.arange(3, 7), 1)[0]
#             else:
#                 num_CAV = num_CAV
#             num_HDV = np.random.choice(np.arange(3, 7), 1)[0]
...
class MergeEnv(AbstractEnv):
    ...
    def _reset(self, num_CAV=0) -> None:
        ...
        if self.config["traffic_density"] == 1:
            ...
            if num_CAV == 0:
                ...
                num_CAV = num_CAV
            num_HDV = np.random.choice(np.arange(0, 2), 1)[0]
            ...
            if num_CAV == 0:
                ...
                num_CAV = num_CAV
            num_HDV = np.random.choice(np.arange(1, 5), 1)[0]
        self._make_vehicles(num_CAV, num_HDV)
        self.action_is_safe = True
        self.T = int(self.config["duration"] * self.config["policy_frequency"])
    ...
    def _make_vehicles(self, num_CAV=4, num_HDV=3) -> None:
        ...
        road = self.road
        other_vehicles_type = utils.class_from_path(self.config["other_vehicles_type"])
        agg_vehicles_type = utils.class_from_path(self.config["agg_vehicle_type"])
        def_vehicles_type = utils.class_from_path(self.config["def_vehicle_type"])
        self.controlled_vehicles = []
            ...
            road.vehicles.append(
                other_vehicles_type(road, road.network.get_lane(("j", "k", 0)).position(
                    spawn_point_m_h.pop(0) + loc_noise.pop(0), 0),
                                    speed=initial_speed.pop(0)))
        road.vehicles.append(
            agg_vehicles_type(road, road.network.get_lane(("j", "k", 0)).position(
                100 + np.random.rand() * 3 - 1.5, 0),
                                speed=np.random.rand() * 2 + 25))
        road.vehicles.append(
            def_vehicles_type(road, road.network.get_lane(("a", "b", 0)).position(
                120 + np.random.rand() * 3 - 1.5, 0),
                              speed=np.random.rand() * 2 + 25))
