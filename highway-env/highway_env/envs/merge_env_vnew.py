...
class MergeEnvNew(AbstractEnv):
    ...
    def _spawn_vehicle(self,
                       ...
                       speed_deviation: float = 1.5,
                       spawn_probability: float = 0.3,
                       ) -> None:
        if random.uniform(0, 1) > spawn_probability:
            return
        road = self.road
        ...
        if random.choice([0, 1]) == 0:
            # 生成主干道车辆
            vehicle = vehicle_type.make_on_lane(road, ("a", "b", 0),
                                                longitudinal=(longitudinal + 10
                                                              + random.gauss(0, position_deviation)),
                                                speed=26 + random.gauss(0, speed_deviation))
            vehicle.id = self.road.vehicles[-1].id + 1
        else:
            # 生成匝道车辆
            vehicle = vehicle_type.make_on_lane(road, ("j", "k", 0),
    ...
    def _clear_vehicles(self) -> None:
        is_leaving = lambda vehicle: "c" in vehicle.lane_index[0] and "d" in vehicle.lane_index[1] \
                                     and vehicle.lane.local_coordinates(vehicle.position)[0] \
                                     >= vehicle.lane.length - 1* vehicle.LENGTH
...
class MergeEnvMARLNew(MergeEnvNew):
    @classmethod
    def default_config(cls) -> dict:
        config = super().default_config()
        config.update({
...
register(
    id='new-merge-v0',
    entry_point='highway_env.envs.merge_env_vnew:MergeEnvNew',
)

register(
    id='new-merge-multi-agent-v0',
    entry_point='highway_env.envs.merge_env_vnew:MergeEnvMARLNew',
)