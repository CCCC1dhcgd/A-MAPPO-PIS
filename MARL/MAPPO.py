...
#         ##############
#         # env_ = self.env
#         # env_copy = copy.deepcopy(env_.env)
#         # n_points = int(env_.env.config["simulation_frequency"] // env_.env.config["policy_frequency"]) * \
#         #            env_.env.config[
#         #                "n_step"]
#         ########################
#
...
#             ###########################
#             # # 车辆轨迹初始化
#             # for v in env_.env.road.vehicles: #原环境
#             #     v.trajectories = []
#             # for v in env_copy.road.vehicles:
#             #     v.trajectories = []
#             # # 轨迹生成
#             # self.generate_trajectory(env_copy.controlled_vehicles, env_copy, action, n_points) # 生成CAVs即其周围车辆的轨迹
#             # for v_id in range(len(env_copy.road.vehicles)):
#             #    env_.env.road.vehicles[v_id].trajectories = env_copy.road.vehicles[v_id].trajectories
#             ##################################
...
#
#     # TODO：新加未来轨迹生成模块
#     def generate_trajectory(self, vehicls, env_copy, action, n_points=5):
#         """
#         生成未来轨迹
#         :param vehicls: 受控的智能体
#         :param env_copy: 环境变量
#         :param action: 策略动作
#         :param n_points: 生成的时间步数
#         """
#         for t in range(n_points):
#             for v in vehicls:
#                 mdp_controller(v, env_copy, action[v.id])
#
...
class MAPPO:
    ...
    def load(self, model_dir, global_step=None, train_mode=False):
        ...
        if save_file is not None:
            file_path = model_dir + save_file
            checkpoint = th.load(file_path, map_location='cpu')
            print('Checkpoint loaded: {}'.format(file_path))
            # logging.info('Checkpoint loaded: {}'.format(file_path))
            self.actor.load_state_dict(checkpoint['model_state_dict'])
            if train_mode:
