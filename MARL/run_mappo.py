...
def parse_args():
    ...
    parser.add_argument('--model-dir', type=str, required=False,
                        default=r'D:\py_learning\marl_cav_new\MARL_AD_U-20240629T131712Z-001\MARL_AD_U\MARL\results\MAPPO-PIS_hard_0 - 副本', help="pretrained model path")
...
def evaluate(args):
    ...
    rewards, (vehicle_speed, vehicle_position), steps, avg_speeds, PETs, collision_rates = mappo.evaluation(env, video_dir, len(seeds),
                                                                                     is_train=False)
    rewards_mu, rewards_std = agg_double_list(rewards)
    ...
    # 合并所有展平的数组
    # vehicle_speed_combined = np.concatenate(vehicle_speed_flattened)

    # 将每个子数组展平为一维数组
    vehicle_position_flattened = [arr.flatten() for arr in vehicle_position]

    # 合并所有展平的数组
    # vehicle_position_combined = np.concatenate(vehicle_position_flattened)
    ...
    np.save(eval_logs + '/{}'.format('vehicle_speed'), np.array(vehicle_speed_flattened))
    np.save(eval_logs + '/{}'.format('vehicle_position'), np.array(vehicle_position_flattened))
